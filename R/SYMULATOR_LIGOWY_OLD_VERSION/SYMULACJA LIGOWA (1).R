...
punkty_do_utrzymania[h]<-tabela_koncowa[tabela_koncowa$Miejsce==15,"Punkty"]
...
ggplot(data=data.frame(punkty_do_utrzymania),aes(x=punkty_do_utrzymania))+
  ...
  scale_x_continuous(limits = c(30,45),breaks=30:45)+
  ggtitle("MINIMUM PUNKTÓW POTRZEBNYCH DO UTRZYMANIA")

quantile(punkty_do_utrzymania,c(0.01))
mean(punkty_do_utrzymania)

#write.table(t(rbind(szanse_spadek,szanse_awans,szanse_pozostanie,szanse_utrzymanie)),file="tabela.csv",sep=";",col.names = T,row.names = T)
