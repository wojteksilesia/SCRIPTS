...
PROCEDURE ADD_REMOVE_WAITING_LIST(inStatus IN NUMBER,ivOpening IN VARCHAR2,ivColor IN VARCHAR2,ivLink IN VARCHAR2);
...
PROCEDURE ADD_REMOVE_WAITING_LIST(inStatus IN NUMBER,ivOpening IN VARCHAR2,ivColor IN VARCHAR2,ivLink IN VARCHAR2) IS
	lnId NUMBER;
BEGIN
	IF inStatus=-1 THEN
		DELETE FROM WAITING_LIST WHERE OPENING=ivOpening;
		COMMIT;
	ELSIF inStatus=1 THEN
		SELECT NVL(MAX(ID)+1,1) INTO lnId FROM WAITING_LIST;
		INSERT INTO WAITING_LIST
		(ID,OPENING,COLOR,LINK,DATE_ADDED)
		VALUES
		(lnId,ivOpening,ivColor,ivLink,SYSDATE);
		COMMIT;
	END IF;
